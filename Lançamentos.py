...
if escolha == 'Lançar':
    ...
    for i, item in enumerate(select_nome_id_staff):
        if i < len(select_nome_id_staff) / 2:
            done = col1.checkbox(str(item[1]), key=str(i))
        else:
            done = col2.checkbox(str(item[1]), key=str(i))
